...
#check time_space.mk_time_transform_to
variables (f1 f2 : time_frame) (s1 : time_space f1) (s2 : time_space f2)
#check s1.mk_time_transform_to s2
#check s1
#check camera_os_system_time_ms.coords

#check (camera_os_system_time_ms.coords).mk_time_transform_to (camera_os_system_time_seconds.coords)
--
def imu_callback_sync_v1 : timestamped camera_os_system_time_ms.coords (displacement3d camera.coords) → punit := 
...
-/
  let elapsed_camera_ms : duration camera_os_system_time_ms.coords
    := ((camera_os_system_time_ms.coords).mk_time_transform_to camera_os_system_time_seconds.coords : 
    time_transform _ _).transform_duration 
      ((dataframe_time -ᵥ _camera_time_base : duration camera_os_system_time_ms.coords)) in
    /-
  ...
  let imu_msgs : list (timestamped camera_os_system_time_seconds.coords (displacement3d camera.coords)) := [imu_data] in
...
--double elapsed_camera_ms = (/*ms*/ frame_time - /*ms*/ _camera_time_base) / 1000.0;
  ...
  let elapsed_camera_ms : duration camera_hardware_time.coords
    := ((camera_os_system_time_ms.coords : time_space _ ).mk_time_transform_to (camera_os_system_time_seconds.coords) : time_transform _ _).transform_duration 
    ((dataframe_time -ᵥ _camera_time_base : duration camera_os_system_time_ms.coords)) in
    /-
        auto crnt_reading = *(reinterpret_cast<const float3*>(frame.get_data()));
        Eigen::Vector3d v(crnt_reading.x, crnt_reading.y, crnt_reading.z);
